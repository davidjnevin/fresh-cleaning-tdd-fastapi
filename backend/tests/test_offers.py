...
class TestOffersRoutes:
    ...
    async def test_create_offer_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.post(app.url_path_for("offers:create-offer", cleaning_id=1))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_list_offer_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.get(app.url_path_for("offers:list-offers-for-cleaning", cleaning_id=1))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_get_offer_from_user_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.get(app.url_path_for("offers:get-offer-from-user", cleaning_id=1, username="bradpitt"))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_accept_offer_from_user_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.put(app.url_path_for("offers:accept-offer-from-user", cleaning_id=1, username="bradpitt"))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_cancel_offer_from_user_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.put(app.url_path_for("offers:cancel-offer-from-user", cleaning_id=1))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_recind_offer_from_user_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
