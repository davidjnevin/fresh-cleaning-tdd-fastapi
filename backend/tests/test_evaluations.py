...
from app.models.evaluation import (EvaluationAggregate, EvaluationCreate,
                                   EvaluationInDB, EvaluationPublic)
...
class TestEvaluationRoutes:
    async def test_create_evaluation_for_cleaner_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.post(
            app.url_path_for("evaluations:create-evaluation-for-cleaner", cleaning_id=1, username="bradpitt")
        )
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_get_evaluation_for_cleaner_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.get(
            app.url_path_for("evaluations:get-evaluation-for-cleaner", cleaning_id=1, username="bradpitt")
        )
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_list_evaluations_for_cleaner_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
        res = await client.get(app.url_path_for("evaluations:list-evaluations-for-cleaner", username="bradpitt"))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_get_stats_for_cleaner_route_exist(self, app: FastAPI, client: AsyncClient) -> None:
