...
class TestCleaningsRoutes:
    ...
    async def test_create_cleanings_route_exist(
        self, app: FastAPI, client: AsyncClient
    ) -> None:
        res = await client.post(app.url_path_for("cleanings:create-cleaning"), json={})
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_get_cleaning_by_id_route_exist(
        self, app: FastAPI, client: AsyncClient
    ) -> None:
    ...
    async def test_list_all_user_cleanings_route_exist(
        self, app: FastAPI, client: AsyncClient
    ) -> None:
        res = await client.get(app.url_path_for("cleanings:list-all-user-cleanings"))
        assert res.status_code != status.HTTP_404_NOT_FOUND

    async def test_update_cleaning_by_id_route_exist(
        self, app: FastAPI, client: AsyncClient
    ) -> None:
    ...
    async def test_delete_cleaning_by_id_route_exist(
        self, app: FastAPI, client: AsyncClient
    ) -> None:
...
class TestUpdateCleaning:
    ...
    async def test_update_cleaning_with_valid_input(
        ...
        for attr, value in updated_cleaning.dict().items():
            if (
                attr not in attrs_to_change and attr != "updated_at"
            ):  # and attr != "created_at":
